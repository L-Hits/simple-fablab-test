...
const AccountRecovery = () => {
  ...
  const requestVerificationCode = async () => {
    try {
      ...
      await axiosInstance.post(
        ...
        {
          email,
          purpose: "accountRecovery", // 계정 복구용 인증번호 요청
          id,
        },
        {
          headers: {
  ...
  // 계정 복구 API 시작
  const accountrecoverybtn = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      // 요청 성공 처리
      alert(response.data.message || "계정 복구가 완료되었습니다!");
      navigate("/Login");
    } catch (error) {
      // 요청 실패 처리
      if (axios.isAxiosError(error) && error.response) {
        alert(
          "계정 복구 실패: " + (error.response.data?.message || "알 수 없는 오류")
        );
      } else {
        console.error("요청 오류:", (error as Error).message);
        alert("예기치 않은 오류가 발생했습니다. 나중에 다시 시도해 주세요.");
    ...
    >
      <FormControl>
        ...
        >
          ...
          <Button variant="contained" fullWidth onClick={accountrecoverybtn}>
            계정 복구
          </Button>
